# Task: Add Multi-Session Thread Support to Errandd
...
1. **Create `src/sessionManager.ts`** — Manages thread-to-session mappings
   - Store in `.claude/errandd/sessions.json`
...
When completely finished, run: openclaw system event --text "Done: Errandd multi-session thread support implemented" --mode now

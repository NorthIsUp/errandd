...
1. **Block home-directory starts (CRITICAL, BLOCKER)**:
   ...
   - Tell the user exactly:
     - "CRITICAL BLOCKER: For security reasons, close this session and start a new one from the folder you want to initialize ClawdCode in."
...
3. **Check existing config**: Read `.claude/clawdcode/settings.json` (if it exists). Determine which sections are already configured:
...
5. **Ask setup questions**:
   ...
   - **Model** (always ask if `model` is empty/unset): "Which Claude model should ClawdCode use?" (header: "Model", options: "opus (default)", "sonnet", "haiku", "glm")
   ...
   Update `.claude/clawdcode/settings.json` with their answers.

6. **Launch/start action**:
   ```bash
   mkdir -p .claude/clawdcode/logs && nohup bun run ${CLAUDE_PLUGIN_ROOT}/src/index.ts start --web > .claude/clawdcode/logs/daemon.log 2>&1 & echo $!
   ```
   Use the description "Starting ClawdCode server" for this command.
   Wait 1 second, then check `cat .claude/clawdcode/logs/daemon.log`. If it contains "Aborted: daemon already running", tell the user and exit.
   - Read `.claude/clawdcode/settings.json` for `web.port` (default `4632` if missing) and `web.host` (default `127.0.0.1`).
...
7. **Capture session ID**: Read `.claude/clawdcode/session.json` and extract the `sessionId` field. This is the shared Claude session used by the daemon for heartbeat, jobs, Telegram, and Discord.
...
### Settings — `.claude/clawdcode/settings.json`
```json
{
  "model": "opus",
  "api": "",
...
- Heartbeat template override (optional) — create `.claude/clawdcode/prompts/HEARTBEAT.md` to replace the built-in heartbeat template for this project.
...
### Jobs — `.claude/clawdcode/jobs/<name>.md`
...
- Jobs are loaded at daemon startup from `.claude/clawdcode/jobs/`

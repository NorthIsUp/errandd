...
1. Read `.claude/errandd/settings.json`.
...
3. Also list any cron jobs from the configured jobs directory (see `jobsDir` in settings, default: `.claude/errandd/jobs/`) with their name, schedule, and notify mode.
...
2. Read `.claude/errandd/settings.json`.
...
1. Read `.claude/errandd/settings.json` and show the current prompt.
...
3. Read `.claude/errandd/settings.json`.
...
2. Otherwise, use **AskUserQuestion**: "Which Claude model should Errandd use?" (header: "Model", options: "opus (default)", "sonnet", "haiku", "glm")
3. Read `.claude/errandd/settings.json`.
...
1. If token is in `$ARGUMENTS`, use it directly.
2. Otherwise, use **AskUserQuestion**: "What API token should Errandd use for glm?" (header: "API token", options: let user type via Other)
3. Read `.claude/errandd/settings.json`.
...
2. Otherwise, use **AskUserQuestion**: "Which fallback model should Errandd use?" (header: "Fallback model", options: "glm (Recommended)", "sonnet", "haiku")
3. Read `.claude/errandd/settings.json`.
...
1. If token is in `$ARGUMENTS`, use it directly.
2. Otherwise, use **AskUserQuestion**: "What API token should Errandd use for fallback model?" (header: "Fallback API token", options: let user type via Other)
3. Read `.claude/errandd/settings.json`.
...
2. Otherwise, use **AskUserQuestion**: "What timezone should Errandd use?" (header: "Timezone", options: "UTC (Recommended)", "America/New_York", "Europe/London")
3. Read `.claude/errandd/settings.json`.
...
Location: `.claude/errandd/settings.json`

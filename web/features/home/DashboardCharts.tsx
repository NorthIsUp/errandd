...
export function DashboardCharts({ allUsage }: Props) {
  ...
  const range: TimeRange = (
    RANGES.includes(rangeRaw as TimeRange) ? rangeRaw : "24h"
  ) as TimeRange;
  ...
  const loadTimeline = useCallback(async (r: TimeRange) => {
    ...
    try {
      ...
      console.error(
        "[DashboardCharts] Failed to load timeline data:",
        msg,
        err,
      );
  ...
  const activityData = useMemo<Record<string, string | number>[]>(() => {
    ...
    const rows: Row[] = timelineBuckets.map((b) => ({
    ...
    <div className={styles.dashWrap}>
      ...
      {timelineError !== null && (
        <div className={styles.errorBanner} role="alert">
          Failed to load chart data: {timelineError}
        </div>
      )}
        ...
        <Card glass>
          ...
          <CardContent>
            {activityData.length === 0 ? (
              <div className={styles.emptyState}>No activity in this range</div>
            ) : (
              <AreaChart
                data={activityData}
                xKey="ts"
        ...
        <Card glass>
          ...
          <CardContent>
            {densityData.length === 0 ? (
              <div className={styles.emptyState}>No scheduled jobs</div>
            ) : (
              <BarChart
                data={densityData}
                xKey="hour"
        ...
        <Card glass>
          ...
          <CardContent>
            {topJobsData.length === 0 ? (
              <div className={styles.emptyState}>
                No job token data in this range
              </div>
            ) : (
              <BarChart
                data={topJobsData}
